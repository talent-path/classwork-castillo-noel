...
export class ChessBoard implements Board {
    ...
    makeMove: (toMake: Move) => Board = toMake => {
        ...
        // check enpassant
        if (oldPiece != null && oldPiece.kind === PieceType.Pawn && Math.abs(toMake.from.row - toMake.to.row) === 2) {
            if (oldPiece.isWhite) {
    ...
    generateMoves(): Move[] {
        ...
        for (let kingMove of kingMoves) {
            ...
            inCheck = King.adjacentKing(this, kingMove.to, kingMove.from.col === whiteKingLoc.col && kingMove.from.row === whiteKingLoc.row);

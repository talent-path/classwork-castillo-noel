...
//        int[] bothHands = { 1, 5, 9, 13, 17, 2, 21, 22, 23, 24  };

        //                   Full House(7) vs Royal Flush(10)
        //                   2D  3D 4D 5D  6D  7S  7D  7C  7H  8S
        int[] bothHands = { 0, 2, 1, 17, 18, 36, 40, 44, 48, 32  };
...
//        Map<Suit, Integer> suitCountsH2 = hand2.countSuits();
        ...
        for (FaceValue key : hand1.getValueCounts().keySet()) {
            System.out.println(key + ": " + hand1.getValueCounts().get(key));

        }

        for (Suit key : hand1.getSuitCounts().keySet()) {
            System.out.println(key + ": " + hand1.getSuitCounts().get(key));
        }


        for (FaceValue key : hand2.getValueCounts().keySet()) {
            System.out.println(key + ": " + hand2.getValueCounts().get(key));

        }

        for (Suit key : hand2.getSuitCounts().keySet()) {
            System.out.println(key + ": " + hand2.getSuitCounts().get(key));
